...
var defaultOptions = {
  ...
  getChunkSize: function getChunkSize(file) {
        ...
        SRTlib.send('], "end": "defaultOptions.getChunkSize.getChunkSize"},');
...
var MultipartUploader = (function () {
  ...
  function MultipartUploader(file, options) {
    ...
    this.createdPromise.catch(function () {
            ...
            SRTlib.send('], "end": "createdPromise.catch"},');

    });
        SRTlib.send('], "end": "MultipartUploader"},');
  ...
  _proto._initChunks = function _initChunks() {
    ...
    this.chunkState = chunks.map(function () {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._initChunks._initChunks.chunkState"},');
      ...
      return {
        ...
        SRTlib.send('], "end": "MultipartUploader._proto._initChunks._initChunks"},');
  ...
  _proto._createUpload = function _createUpload() {
    ...
    this.createdPromise = Promise.resolve().then(function () {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._createUpload._createUpload.createdPromise.then"},');

      return _this.options.createMultipartUpload();
            SRTlib.send('], "end": "MultipartUploader._proto._createUpload._createUpload.createdPromise.then"},');

    });
        SRTlib.send('], "end": "MultipartUploader._proto._createUpload._createUpload"},');
    ...
    return this.createdPromise.then(function (result) {
      ...
      if (!valid) {
                SRTlib.send('], "end": "MultipartUploader._proto._createUpload._createUpload.ReturnStatement.createdPromise.then.catch.createdPromise.then"},');
      ...
      _this._onError(err);
            SRTlib.send('], "end": "MultipartUploader._proto._createUpload._createUpload.ReturnStatement.createdPromise.then.catch"},');
  ...
  _proto._resumeUpload = function _resumeUpload() {
    ...
    var _this2 = this;
        SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload"},');
    ...
    return Promise.resolve().then(function () {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload.ReturnStatement.then.then.catch.then.then.then"},');
      ...
      parts.forEach(function (part) {
        ...
        if (!_this2.parts.some(function (p) {
                    ...
                    SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload.ReturnStatement.then.then.catch.then.then._this2.parts.some"},');
          ...
          _this2.parts.push({
            ...
            ETag: part.ETag
                ...
                SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload.ReturnStatement.then.then.catch.then.then"},');

      });
      _this2._uploadParts();
            SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload.ReturnStatement.then.then.catch.then.then2"},');
      ...
      _this2._onError(err);
            SRTlib.send('], "end": "MultipartUploader._proto._resumeUpload._resumeUpload.ReturnStatement.then.then.catch"},');
  ...
  _proto._uploadParts = function _uploadParts() {
    ...
    var _this3 = this;
    if (this.isPaused) {
            SRTlib.send('], "end": "MultipartUploader._proto._uploadParts._uploadParts2"},');
    ...
    if (this.chunkState.every(function (state) {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._uploadParts._uploadParts.chunkState.every"},');
    ...
    candidates.forEach(function (index) {
      ...
      _this3._uploadPart(index);
            SRTlib.send('], "end": "MultipartUploader._proto._uploadParts._uploadParts"},');
  ...
  _proto._uploadPart = function _uploadPart(index) {
    ...
    this.chunkState[index].busy = true;
        SRTlib.send('], "end": "MultipartUploader._proto._uploadPart._uploadPart"},');
    ...
    return Promise.resolve().then(function () {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._uploadPart._uploadPart.ReturnStatement.then.then.then.then.then.then"},');
      ...
      if (!valid) {
                SRTlib.send('], "end": "MultipartUploader._proto._uploadPart._uploadPart.ReturnStatement.then.then.then.then.then"},');
      ...
      _this4._uploadPartBytes(index, url, headers);
            SRTlib.send('], "end": "MultipartUploader._proto._uploadPart._uploadPart.ReturnStatement.then.then.then"},');
      ...
      _this4._onError(err);
            SRTlib.send('], "end": "MultipartUploader._proto._uploadPart._uploadPart.ReturnStatement.then.then.then2"},');
  ...
  _proto._onPartProgress = function _onPartProgress(index, sent, total) {
    ...
    var totalUploaded = this.chunkState.reduce(function (n, c) {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._onPartProgress._onPartProgress.totalUploaded.chunkState.reduce"},');
    ...
    this.options.onProgress(totalUploaded, this.file.size);
        SRTlib.send('], "end": "MultipartUploader._proto._onPartProgress._onPartProgress"},');
  ...
  _proto._onPartComplete = function _onPartComplete(index, etag) {
    ...
    this._uploadParts();
        SRTlib.send('], "end": "MultipartUploader._proto._onPartComplete._onPartComplete"},');
  ...
  _proto._uploadPartBytes = function _uploadPartBytes(index, url, headers) {
    ...
    if (headers) {
      Object.keys(headers).map(function (key) {
        ...
        xhr.setRequestHeader(key, headers[key]);
                SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes.map"},');
    ...
    xhr.upload.addEventListener('progress', function (ev) {
      ...
      if (!ev.lengthComputable) {
                SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes.xhr.upload.addEventListener"},');
    ...
    xhr.addEventListener('abort', function (ev) {
      ...
      _this5.chunkState[index].busy = false;
            SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes"},');
    ...
    xhr.addEventListener('load', function (ev) {
      ...
      if (ev.target.status < 200 || ev.target.status >= 300) {
        _this5._onError(new Error('Non 2xx'));
                SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes2"},');
    ...
    xhr.addEventListener('error', function (ev) {
      ...
      _this5._onError(error);
            SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes3"},');

    });
    xhr.send(body);
        SRTlib.send('], "end": "MultipartUploader._proto._uploadPartBytes._uploadPartBytes4"},');
  ...
  _proto._completeUpload = function _completeUpload() {
    ...
    this.parts.sort(function (a, b) {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload.parts.sort"},');

      return a.PartNumber - b.PartNumber;
            SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload.parts.sort"},');

    });
        SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload"},');
    ...
    return Promise.resolve().then(function () {
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload.ReturnStatement.then.then.then"},');
      ...
      _this6.options.onSuccess(result);
            SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload.ReturnStatement.then.then"},');
      ...
      _this6._onError(err);
            SRTlib.send('], "end": "MultipartUploader._proto._completeUpload._completeUpload.ReturnStatement.then.then2"},');
  ...
  _proto._abortUpload = function _abortUpload() {
    ...
    this.uploading.slice().forEach(function (xhr) {
      ...
      xhr.abort();
            SRTlib.send('], "end": "MultipartUploader._proto._abortUpload._abortUpload.uploading.slice.forEach"},');
    ...
    this.createdPromise.then(function () {
      ...
      _this7.options.abortMultipartUpload({
        ...
        uploadId: _this7.uploadId
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._abortUpload._abortUpload.createdPromise.then"},');
            ...
            SRTlib.send('], "end": "MultipartUploader._proto._abortUpload._abortUpload.createdPromise.then2"},');

    });
    this.uploading = [];
        SRTlib.send('], "end": "MultipartUploader._proto._abortUpload._abortUpload"},');
  ...
  _proto._onError = function _onError(err) {
    ...
    this.options.onError(err);
        SRTlib.send('], "end": "MultipartUploader._proto._onError._onError"},');
  ...
  _proto.start = function start() {
    ...
    if (this.uploadId) {
      ...
      this._createUpload();
        ...
        SRTlib.send('], "end": "MultipartUploader._proto.start.start"},');
  ...
  _proto.pause = function pause() {
    ...
    inProgress.forEach(function (xhr) {
      ...
      xhr.abort();
            SRTlib.send('], "end": "MultipartUploader._proto.pause.pause"},');

    });
    this.isPaused = true;
        SRTlib.send('], "end": "MultipartUploader._proto.pause.pause2"},');
  ...
  _proto.abort = function abort(opts) {
    ...
    if (!really) {
            SRTlib.send('], "end": "MultipartUploader._proto.abort.abort"},');
