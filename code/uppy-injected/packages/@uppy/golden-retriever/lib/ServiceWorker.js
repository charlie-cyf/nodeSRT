...
self.addEventListener('install', function (event) {
  ...
  event.waitUntil(Promise.resolve().then(function () {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"self.addEventListener.event.waitUntil.Promise.resolve.then","fileName":"${__filename}","paramsNumber":0},`);

        SRTlib.send('{"type":"FUNCTIONEND","function":"self.addEventListener.event.waitUntil.Promise.resolve.then"},');
...
function sendMessageToAllClients(msg) {
  ...
  clients.matchAll().then(function (clients) {
    ...
    clients.forEach(function (client) {
            SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"clients.matchAll.then.clients.forEach","fileName":"${__filename}","paramsNumber":1},`);

      client.postMessage(msg);
            SRTlib.send('{"type":"FUNCTIONEND","function":"clients.matchAll.then.clients.forEach"},');
