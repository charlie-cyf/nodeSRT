...
var AttachFileInputs = (function (_Plugin) {
  ...
  _proto.handleChange = function handleChange(event) {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"AttachFileInputs._proto.handleChange","fileName":"${__filename}","paramsNumber":1},`);

    this.addFiles(event.target);
        SRTlib.send('{"type":"FUNCTIONEND","function":"AttachFileInputs._proto.handleChange"},');

  };
  _proto.addFiles = function addFiles(input) {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"AttachFileInputs._proto.addFiles","fileName":"${__filename}","paramsNumber":1},`);

    var _this2 = this;
    var files = toArray(input.files);
    files.forEach(function (file) {
            SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"AttachFileInputs._proto.addFiles.addFiles.files.forEach","fileName":"${__filename}","paramsNumber":1},`);
      ...
      try {
        ...
        if (!err.isRestriction) {
          _this2.uppy.log(err);
            ...
            SRTlib.send('{"type":"FUNCTIONEND","function":"AttachFileInputs._proto.addFiles.addFiles.files.forEach"},');

    });
        SRTlib.send('{"type":"FUNCTIONEND","function":"AttachFileInputs._proto.addFiles"},');

  };
  _proto.install = function install() {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"AttachFileInputs._proto.install","fileName":"${__filename}","paramsNumber":0},`);

    var _this3 = this;
    this.el = findDOMElement(this.opts.target);
    if (!this.el) {
            SRTlib.send('{"type":"FUNCTIONEND","function":"AttachFileInputs._proto.install"},');
  ...
  _proto.uninstall = function uninstall() {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"AttachFileInputs._proto.uninstall","fileName":"${__filename}","paramsNumber":0},`);
    ...
    this.inputs = null;
        SRTlib.send('{"type":"FUNCTIONEND","function":"AttachFileInputs._proto.uninstall"},');
