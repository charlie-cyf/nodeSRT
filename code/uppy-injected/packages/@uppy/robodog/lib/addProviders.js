  ...
  names.forEach(function (name) {
    ...
    if (has(remoteProviders, name)) {
      ...
      var expectedNameString = validNames.sort().map(function (validName) {
                SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"names.forEach.expectedNameString.validNames.sort.map.join.validNames.sort.map","fileName":"${__filename}","paramsNumber":1},`);
