...
const isForbiddenHeader = header => {
    SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":false,"function":"isForbiddenHeader","fileName":"/packages/@uppy/companion/lib/server/header-blacklist.js","paramsNumber":1},`);

  const headerLower = header.toLowerCase();
  const forbidden = forbiddenNames.indexOf(headerLower) >= 0 || forbiddenRegex.findIndex(regex => {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"forbidden.forbiddenRegex.findIndex","fileName":"/packages/@uppy/companion/lib/server/header-blacklist.js","paramsNumber":1},`);
...
module.exports = headers => {
    SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"module.exports","fileName":"/packages/@uppy/companion/lib/server/header-blacklist.js","paramsNumber":1},`);
  ...
  Object.keys(headersCloned).forEach(header => {
        SRTlib.send(`{"type":"FUNCTIONSTART","anonymous":true,"function":"Object.keys.forEach","fileName":"/packages/@uppy/companion/lib/server/header-blacklist.js","paramsNumber":1},`);
