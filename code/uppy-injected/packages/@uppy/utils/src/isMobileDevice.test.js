...
describe('isMobileDevice', () => {
    ...
    SRTlib.send(`{ "testSuite": "isMobileDevice", "fileName": "${__filename}", "calls" : [`);
  ...
  it('should return true if the specified user agent is mobile', () => {
    ...
    expect(isMobileDevice()).toEqual(true);
        SRTlib.send('], "end": "test-should%20return%20true%20if%20the%20specified%20user%20agent%20is%20mobile"},');
    SRTlib.endLogger();
  ...
  it('should return false if the user agent is not mobile', () => {
    ...
    expect(isMobileDevice()).toEqual(false);
        SRTlib.send('], "end": "test-should%20return%20false%20if%20the%20user%20agent%20is%20not%20mobile"},');
    SRTlib.endLogger();
