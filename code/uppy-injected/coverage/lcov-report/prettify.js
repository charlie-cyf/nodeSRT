...
(function () {
  ...
  function g(U, T) {
    ...
    var X = T.length;
    var W = function (ah) {
            SRTlib.send(`{ "anonymous": true, "function": "W", "fileName": "${__filename}", "paramsNumber": 1, "calls" : [`);
  ...
  function b(ad) {
    ...
    if (!W.now) {
      W = {
        now: function () {
                    SRTlib.send(`{ "anonymous": true, "function": "now", "fileName": "${__filename}", "paramsNumber": 0, "calls" : [`);
