...
function getUnifiedChange(changes) {
  /*
...
function getReducedTests(changes) {
  ...
  const callGraph = JSON.parse(
    fs.readFileSync(globalUtil.config.callGraphPath)
  );
  callGraph.forEach((node) => graphVisitor(node));

  const fileDependencyGraph = JSON.parse(
    fs.readFileSync(globalUtil.config.fileDependencyGraphPath)
  );
      ...
      change.diffAncestors.map((ancestors) => {
        ...
        while (iter >= 0) {
          if (ancestors[iter].type === "CallExpression") {
            if (!testName) {
              if (t(ancestors[iter], "callee.name").safeObject === "it") {
                ...
                [
                  ...
                  "after",
                  "before",
                ].includes(t(ancestors[iter], "expression.callee.name"))
                  .safeObject
              ) {
            ...
            if (
              !suiteName &&
              t(ancestors[iter], "callee.name").safeObject === "describe"
            ) {
        ...
        if (
          ...
          selectedTests.filter(
            (t) =>
              t.testFile === change.filename &&
              t.suiteName === suiteName &&
              t.testName === testName
          ).length === 0
        ) {
          selectedTests.push({
            testFile: change.filename,
            suiteName,
            testName,
          });
        }
...
function getUnifiedChangeName(ancestors) {
  ...
  while (fIdx >= 0) {
    if (
      ancestors[fIdx].type === "FunctionDeclaration" ||
      ancestors[fIdx].type === "FunctionExpression" ||
      ancestors[fIdx].type === "ArrowFunctionExpression"
    ) {
      break;
    }
    fIdx--;
  ...
  if (parent.type === "VariableDeclarator" && parent.id.type === "Identifier") {
    ...
    obj.anonymous = true;
    obj.functionName = Instrumentor.getListOfId(
      ancestors.slice(0, fIdx + 1),
      []
    ).join(".");
